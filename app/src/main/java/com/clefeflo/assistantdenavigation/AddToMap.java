...
public class AddToMap {
    ...
    public static void calcLiaisons(String point) {
        pointsConnectes.clear(); // on vide la liste, pour ne pas avoir
        // d'anciens points residuels
        poidsLiaisons.clear();
        for (Liaison aLiaison : liaison) {
            // on teste le point 1 de toutes les variables Liaison du tableau
            if (aLiaison.getPoint1().equals(point)) {
                // si le point est celui entré en paramètre
                pointsConnectes.add(aLiaison.getPoint2());
                // on ajoute le point 2 de la variable Liaison
                poidsLiaisons.add(aLiaison.getPoids());
        ...
        for (Liaison aLiaison : liaison) {
            ...
            // chacune)
            if (aLiaison.getPoint2().equals(point)) {
                pointsConnectes.add(aLiaison.getPoint1());
                poidsLiaisons.add(aLiaison.getPoids());
            }
        }
    }
